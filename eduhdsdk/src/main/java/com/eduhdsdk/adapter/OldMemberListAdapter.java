...
public class OldMemberListAdapter extends BaseAdapter {
    ...
    public OldMemberListAdapter(Context context, ArrayList<RoomUser> userList) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (userList.size() > 0) {
            if (convertView == null) {
                ...
                convertView = LayoutInflater.from(context).inflate(R.layout.old_layout_member_list_item, null);
            ...
            if (user != null) {
                ...
                if (user.role == 1) {
                    holder.iv_no_speak.setVisibility(View.GONE);
                }else {
                    holder.iv_no_speak.setVisibility(View.VISIBLE);
                }
                ...
                if (user.properties.containsKey("devicetype")) {
                    ...
                    switch (type) {
                        case "AndroidPad":
                            if (udpstate > 1) {
                                holder.im_type.setImageResource(R.drawable.old_androidpad_red);
                            } else {
                                holder.im_type.setImageResource(R.drawable.old_android_hd);
                            }
                            break;
                        case "iPad":
                            if (udpstate > 1) {
                                holder.im_type.setImageResource(R.drawable.old_ipad_icon_red);
                            } else {
                                holder.im_type.setImageResource(R.drawable.old_ipad_hd);
                            }
                            break;
                        case "AndroidPhone":
                            if (udpstate > 1) {
                                holder.im_type.setImageResource(R.drawable.old_android_red);
                            } else {
                                holder.im_type.setImageResource(R.drawable.old_android);
                            }
                            break;
                        case "iPhone":
                            if (udpstate > 1) {
                                holder.im_type.setImageResource(R.drawable.old_iphone_icon_red);
                            } else {
                                holder.im_type.setImageResource(R.drawable.old_iphone);
                            }
                            break;
                        case "WindowClient":
                            if (udpstate > 1) {
                                holder.im_type.setImageResource(R.drawable.old_win_client_icon_red);
                            } else {
                                holder.im_type.setImageResource(R.drawable.old_win_client);
                            }
                            break;
                        case "WindowPC":
                            if (udpstate > 1) {
                                holder.im_type.setImageResource(R.drawable.old_win_pc_icon_red);
                            } else {
                                holder.im_type.setImageResource(R.drawable.old_win_pc);
                            }
                            break;
                        case "MacClient":
                            if (udpstate > 1) {
                                holder.im_type.setImageResource(R.drawable.old_mac_client_red);
                            } else {
                                holder.im_type.setImageResource(R.drawable.old_mac_client);
                            }
                            break;
                        case "MacPC":
                            if (udpstate > 1) {
                                holder.im_type.setImageResource(R.drawable.old_mac_pc_icon_red);
                            } else {
                                holder.im_type.setImageResource(R.drawable.old_mac_pc);
                            }
                            break;
                        case "AndroidTV":
                            if (udpstate > 1) {
                                holder.im_type.setImageResource(R.drawable.old_icon_tv_red);
                            } else {
                                holder.im_type.setImageResource(R.drawable.old_icon_tv_white);
                            }
                            break;
                        default:
                            if (udpstate > 1) {
                                holder.im_type.setImageResource(R.drawable.old_icon_unknown_red);
                            } else {
                                holder.im_type.setImageResource(R.drawable.old_icon_unknown_white);
                            }
                            break;
                    }
                }
                ...
                if (user.properties.containsKey("disablechat")) {
                    ...
                    if (temp) {
                        holder.iv_no_speak.setImageResource(R.drawable.old_button_speak);
                    } else {
                        holder.iv_no_speak.setImageResource(R.drawable.old_button_close_speak);
                    }
                }
                if (user.role == 1) {
                    holder.img_tool.setEnabled(false);
                    holder.img_tool.setImageResource(R.drawable.old_button_close_editor_unclickable);//可以画图
                } else {
                ...
                if (user.publishState > 0) {//只要有流
                    if (user.role == 0) {
                        holder.img_up_sd.setImageResource(R.drawable.old_button_shangjiangtai);
                    } else if (user.role == 1) {
                        holder.img_up_sd.setImageResource(R.drawable.old_button_shangjiangtai);
                    } else if (user.role == 2) {
                        holder.img_up_sd.setImageResource(R.drawable.old_button_shangjiangtai);
                    ...
                    if (user.role == 0) {
                        holder.img_up_sd.setImageResource(R.drawable.old_button_xiajiangtai);
                    } else if (user.role == 1) {
                        holder.img_up_sd.setImageResource(R.drawable.old_button_xiajiangtai);
                    } else if (user.role == 2) {
                        holder.img_up_sd.setImageResource(R.drawable.old_button_xiajiangtai);
                ...
                if (user.disableaudio) {
                    holder.img_audio.setImageResource(R.drawable.old_button_no_audio);
                } else {
                    if (user.publishState == 1 || user.publishState == 3) {//音频状态改的开启
                        holder.img_audio.setImageResource(R.drawable.old_button_close_audio);
                    } else {
                        holder.img_audio.setImageResource(R.drawable.old_button_audio);
                    }
                }
                if (user.disablevideo) {
                    holder.img_video.setImageResource(R.drawable.old_icon_no_video);
                } else {
                    if (user.publishState == 2 || user.publishState == 3) {//视频状态改的开启
                        holder.img_video.setImageResource(R.drawable.old_button_close_video);
                    } else {
                        holder.img_video.setImageResource(R.drawable.old_button_video);
                    }
                }
                ...
                if (TKRoomManager.getInstance().getMySelf().role != 4) {
                    ...
                    holder.img_hand_up.setOnClickListener(new View.OnClickListener() {
                        @Override
...
//                        }
                    ...
                    holder.img_video.setOnClickListener(
                    ...
                    holder.iv_no_speak.setOnClickListener(
                            new View.OnClickListener() {
                                @Override
                                public void onClick(View v) {
                                    boolean b = false;
