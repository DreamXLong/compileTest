...
public class OldFileListAdapter extends BaseAdapter {
    ...
    public OldFileListAdapter(Context context, String Roomnum) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (arrayList.size() > 0) {
            if (convertView == null) {
                ...
                convertView = LayoutInflater.from(context).inflate(R.layout.old_layout_file_list_item, null);
                ...
                ScreenScale.scaleView(convertView, "oldFilelistAdapter");
            ...
            if (fileDoc != null) {
                ...
                setImgFileType(fileDoc.getFilename(), holder.img_file_type);
                ...
                if (fileDoc.getFileid() == WhiteBoradManager.getInstance().getCurrentFileDoc().getFileid()) {
                    holder.img_eye.setImageResource(R.drawable.old_icon_show_file);
                } else {
                    holder.img_eye.setImageResource(R.drawable.old_icon_file_showing);
                }
    ...
    private void setImgFileType(String filename, ImageView imageView) {
    ...
    private int getFileIcon(String filename) {
        int icon = -1;
        if (filename == null && filename.isEmpty()) {
            icon = R.drawable.old_icon_empty;
        }
        if (filename.toLowerCase().endsWith(".pptx") || filename.toLowerCase().endsWith(".ppt") || filename.toLowerCase().endsWith(".pps")) {
            icon = R.drawable.old_icon_ppt;
        } else if (filename.toLowerCase().endsWith(".docx") || filename.toLowerCase().endsWith(".doc")) {
            icon = R.drawable.old_icon_word;
        } else if (filename.toLowerCase().endsWith(".jpg") || filename.toLowerCase().endsWith(".jpeg")
                || filename.toLowerCase().endsWith(".png") || filename.toLowerCase().endsWith(".gif")
                || filename.toLowerCase().endsWith(".bmp")) {
            icon = R.drawable.old_icon_images;
        } else if (filename.toLowerCase().endsWith(".xls") || filename.toLowerCase().endsWith(".xlsx")
                || filename.toLowerCase().endsWith(".xlt") || filename.toLowerCase().endsWith("xlsm")) {
            icon = R.drawable.old_icon_excel;
        } else if (filename.toLowerCase().endsWith(".pdf")) {
            icon = R.drawable.old_icon_pdf;
        } else if (filename.equals(context.getResources().getString(R.string.share_pad))) {
            icon = R.drawable.old_icon_empty;
        } else if (filename.toLowerCase().endsWith(".txt")) {
            icon = R.drawable.old_icon_text_pad;
        } else if (filename.toLowerCase().endsWith(".zip")) {
            icon = R.drawable.old_icon_h5;
        }
        return icon;
    }
